...
function Opinion() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((error) => {
        const errorMessage = error.response?.data?.message || error.message;
        alert(`댓글을 불러오는 중 문제가 생겼습니다: ${errorMessage}`);
        console.log(error);
      })
      .finally(() => {
        setIsLoading(false);
  ...
  const addComments = () => {
    ...
    axios
      ...
      .catch((error) => {
        const errorMessage = error.response?.data?.message || error.message;
        alert(`댓글 등록 중 문제가 생겼습니다: ${errorMessage}`);
        console.log(error);
      })
      .finally(() => {
        setIsLoading(false);
  ...
  const editComment = () => {
    ...
    axios
      .put(`/api/posts/${postId}/comments/${editingComment}`, {
        content: editText,
      })
      .then((response) => {
        setComments(
      ...
      .catch((error) => {
        const errorMessage = error.response?.data?.message || error.message;
        alert(`댓글 수정 중 문제가 생겼습니다: ${errorMessage}`);
        console.log(error);
      })
      .finally(() => {
        setIsLoading(false);
  ...
  const deleteComment = (commentid) => {
    axios
      ...
      .catch((error) => {
        const errorMessage = error.response?.data?.message || error.message;
        alert(`댓글 삭제 중 문제가 생겼습니다: ${errorMessage}`);
        console.log(error);
      });
  };
  ...
  const getPost = (postId) => {
    axios
      ...
      .then((response) => {
        ...
        setOpenPost(true);
        setPostId(postId);
        if (userId === response.data.authorId) {
          setIsAuthor(true);
        }
      })
      .catch((error) => {
        ...
        <Box sx={{ padding: 4 }}>
          ...
          <Box sx={{ display: "flex", gap: 2, marginTop: 2 }}>
            <Button
              ...
              disabled={!(isAuthor || isLeader || isMentor)}
            >
              본문 수정
            </Button>
            <Button
          ...
          <Typography
            variant="h6"
            sx={{ marginTop: 3, fontWeight: "bold", color: "#6A1B9A" }}
          >
            댓글{" "}
            <span
              style={{
              ...
              comments.map((comment) => (
                <ListItem key={comment.id}>
                  <ListItemText
                    ...
                    } | 최종 수정일: ${comment.updatedAt.split("T")[0]}`}
                  />
                  {/* 댓글 수정 버튼 */}
                  {(comment.authorId === userId || isMentor || isLeader) && (
                    <>
                      <Button
                        onClick={() => {
          ...
          {!editingComment ? (
            ...
            <div>
              ...
              />
              <Button
                onClick={() => editComment(editingComment)}
                variant="contained"
                color="secondary"
                sx={{ marginTop: 2 }}
              >
